  ...
  if (dram_out_queue.empty()) return; 
    ...
    while(entry->req_ops.size()) {
      Op *w_op = entry->req_ops.front();
		if(w_op->pseudo_op == 1)
		{
			// Install TLB entry if this is a pte entry
			// 1st get vpn from the op
		//	cout<< " In fil queue function and its a Pseudo Op"<<endl;		
			uint64_t vpn = w_op->vpn;
		//	cout << "pseudo op vpn: " <<vpn<<endl;
